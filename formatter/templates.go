...
{{- with .RawData }}<pre><code>{{ . }}</code></pre>{{ end }}{{ end }}`
	alertMessageDiskFullTemplateStr = `{{ .StateEmoji }} <b>{{ .StateStr }}</b>
{{- with .Payload }}<p>Rule: <a href="{{ .RuleURL }}">{{ .RuleName }}</a>{{ if .Message }} | {{ .Message }}{{ end }}</p>
<p>Host: {{ .Labels["host"] }}</p>`
...
var (
	...
	alertMessageDiskFullTemplate = htmlTemplate.Must(htmlTemplate.New("alertMessageDiskFull").Funcs(htmlTemplate.FuncMap{
		"RoundValue": roundMetricValue,
	}).Parse(alertMessageDiskFullTemplateStr))
